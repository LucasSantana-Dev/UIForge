...
### Added
- **Anthropic server-side fallback**: Automatic failover to Anthropic when primary provider (Gemini) hits quota/rate-limit errors (429, RESOURCE_EXHAUSTED)
  - Quota error detection with pattern matching in provider-router
  - New `fallback` SSE event type for client awareness of provider switch
...
### Fixed
- **Supabase database advisor warnings**: RLS initplan optimization, permissive policy consolidation, function search_path security, foreign key indexes
- **E2E tests**: Landing page ecosystem count (five to six repos), billing page strict mode selectors
