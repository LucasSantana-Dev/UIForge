...
test.describe('Project Management', () => {
  ...
  test('should filter projects by framework', async ({ page }) => {
    ...
    if (await filterSelect.count() > 0) {
      ...
      const projectCards = page.locator('[data-testid="project-card"]').or(
        page.locator('article').filter({ hasText: /react/i })
      );
  ...
  test('should edit a project', async ({ page }) => {
    ...
    await projectCard.locator('button[aria-label*="menu"]').or(
      projectCard.getByRole('button').filter({ hasText: /more/i })
    ).click();
  ...
  test('should upload project thumbnail', async ({ page }) => {
    ...
    if (await fileInput.count() > 0) {
      ...
      await expect(page.locator('img[alt*="preview"]').or(
  ...
  test('should show empty state when no projects exist', async ({ page }) => {
    ...
    const emptyState = page.getByText(/no projects yet/i).or(
      page.getByText(/get started by creating/i)
    );
  ...
  test('should handle project creation validation errors', async ({ page }) => {
    ...
    await expect(page.getByText(/name.*required/i).or(
      page.getByText(/must be at least/i)
    )).toBeVisible();
  });
  ...
  test('should navigate between grid and list views', async ({ page }) => {
    ...
    const gridViewButton = page.getByRole('button', { name: /grid/i }).or(
    ...
    if (await gridViewButton.count() > 0 && await listViewButton.count() > 0) {
      ...
      await expect(page.locator('[data-view="list"]').or(
        page.locator('.list-view')
      )).toBeVisible();
      
      await gridViewButton.click();
      await expect(page.locator('[data-view="grid"]').or(
