...
test.describe('Component Generation', () => {
  ...
  test('should generate a component', async ({ page }) => {
    ...
    await page.getByLabel(/describe your component/i).fill(
  ...
  test('should validate component name format', async ({ page }) => {
    ...
    for (const name of invalidNames) {
      ...
      await page.getByLabel(/describe your component/i).fill('Test description that is long enough');
      await page.getByRole('button', { name: /generate component/i }).click();
      
      // Should show validation error
      await expect(page.getByText(/valid.*identifier/i).or(
        page.getByText(/invalid.*name/i)
      )).toBeVisible();
  ...
  test('should copy generated code', async ({ page }) => {
    ...
    await page.getByLabel(/describe your component/i).fill('A simple test component for copy functionality');
    await page.getByRole('button', { name: /generate component/i }).click();
  ...
  test('should download generated code', async ({ page }) => {
    ...
    await page.getByLabel(/describe your component/i).fill('A test component for download functionality');
    await page.getByRole('button', { name: /generate component/i }).click();
  ...
  test('should handle generation with tests option', async ({ page }) => {
    ...
    await page.getByLabel(/describe your component/i).fill('Component with test generation enabled');
  ...
  test('should handle rate limiting', async ({ page }) => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      await page.getByLabel(/describe your component/i).fill(`Test component number ${i} for rate limiting`);
      await page.getByRole('button', { name: /generate component/i }).click();
