...
describe('TechDocsPanel', () => {
  it('shows empty state when no URL provided', () => {
    ...
    expect(
      screen.getByText(/No documentation URL configured/i)
    ).toBeInTheDocument();
  });
  ...
  it('shows loading state while fetching', () => {
    ...
    render(
      <TechDocsPanel documentationUrl="https://github.com/org/repo" />
    );
  ...
  it('shows error state with external link fallback', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Unable to load documentation/i)
      ).toBeInTheDocument();
    });
