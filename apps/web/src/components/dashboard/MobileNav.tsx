...
export default function MobileNav({ open, onClose }: MobileNavProps) {
  ...
  return (
    <>
      ...
      <div
        ref={panelRef}
        className="fixed inset-y-0 left-0 flex flex-col w-64 bg-surface-0 z-50"
        role="dialog"
        aria-modal="true"
        aria-label="Mobile navigation"
      >
        <div className="flex items-center justify-between h-16 px-4 border-b border-surface-3">
          <h1 className="text-2xl font-display font-bold text-brand-light">Siza</h1>
          <button
            ref={closeButtonRef}
            type="button"
            onClick={onClose}
            className="p-2 rounded-md text-text-secondary hover:text-text-primary hover:bg-surface-2"
            aria-label="Close navigation"
          >
