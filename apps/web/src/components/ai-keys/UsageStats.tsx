...
import type { UsageStats } from '@/lib/ai-keys';
...
export function UsageStats({ stats }: UsageStatsProps) {
  ...
  const mostRecentlyUsed = Object.entries(stats.lastUsedTimes)
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Overview of your API key usage and activity
        </CardDescription>
      </CardHeader>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center p-4 bg-muted/50 rounded-lg">
            ...
            <div className="text-2xl font-bold text-green-600">
              {Object.values(stats.keysByProvider).reduce((sum, count) => sum + count, 0) - stats.expiredKeys.length}
            </div>
            ...
            <div className="space-y-2">
              {Object.entries(stats.lastUsedTimes)
                ...
                .map(([keyId, lastUsed]) => {
                  ...
                  const provider = Object.keys(AI_PROVIDERS).find(p =>
                    stats.keysByProvider[p as AIProvider] > 0
            ...
            <div>
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                <p className="text-sm text-orange-800 mb-2">
                  You have {stats.expiredKeys.length} API key{stats.expiredKeys.length > 1 ? 's' : ''} that are older than 90 days.
                </p>
                <p className="text-xs text-orange-700">
                  For security reasons, consider updating or replacing expired keys.
                  Most providers recommend rotating keys every 90 days.
                </p>
                <div className="mt-2">
                  <Badge variant="outline" className="text-orange-800 border-orange-300">
                    {stats.expiredKeys.length} key{stats.expiredKeys.length > 1 ? 's' : ''} need attention
                  </Badge>
                </div>
              </div>
            </div>
