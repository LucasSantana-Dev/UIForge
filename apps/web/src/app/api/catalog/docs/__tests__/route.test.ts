...
const mockVerifySession = api.verifySession as jest.MockedFunction<
  typeof api.verifySession
>;
const mockSuccessResponse = api.successResponse as jest.MockedFunction<
  typeof api.successResponse
>;
const mockErrorResponse = api.errorResponse as jest.MockedFunction<
  typeof api.errorResponse
>;
const mockCheckRateLimit = rateLimit.checkRateLimit as jest.MockedFunction<
  typeof rateLimit.checkRateLimit
>;
const mockSetRateLimitHeaders =
  rateLimit.setRateLimitHeaders as jest.MockedFunction<
...
beforeEach(() => {
  ...
  mockErrorResponse.mockImplementation(
    (msg, code) =>
      new Response(JSON.stringify({ error: msg }), { status: code || 500 }),
  );
});
...
describe("Catalog Docs API - /api/catalog/docs", () => {
  ...
  it("returns 400 for unresolvable URL", async () => {
    ...
    await GET(req);
    expect(mockErrorResponse).toHaveBeenCalledWith(
      "Unable to resolve documentation URL",
      400,
    );
  });
  ...
  it("resolves GitHub blob URL to raw content", async () => {
    ...
    const req = makeRequest(
      "/api/catalog/docs?url=" +
        encodeURIComponent(
          "https://github.com/Forge-Space/siza/blob/main/README.md",
        ),
    );
    await GET(req);
  ...
  it("resolves GitHub repo root to default README.md", async () => {
    ...
    const req = makeRequest(
      "/api/catalog/docs?url=" +
        encodeURIComponent("https://github.com/Forge-Space/siza"),
    );
    await GET(req);
    expect(mockFetch).toHaveBeenCalledWith(
      "https://raw.githubusercontent.com/Forge-Space/siza/main/README.md",
  ...
  it("passes through raw.githubusercontent.com URLs", async () => {
    const rawUrl =
      "https://raw.githubusercontent.com/Forge-Space/siza/main/docs/guide.md";
    mockFetch.mockResolvedValue({
      ok: true,
      text: () => Promise.resolve("# Raw"),
    });
    const req = makeRequest(
      "/api/catalog/docs?url=" + encodeURIComponent(rawUrl),
    );
    await GET(req);
  ...
  it("accepts .md URLs from any host", async () => {
    ...
    const req = makeRequest(
      "/api/catalog/docs?url=" +
        encodeURIComponent("https://docs.example.com/api.md"),
    );
    await GET(req);
    expect(mockFetch).toHaveBeenCalledWith(
  ...
  it("returns fetched content with source and truncated flag", async () => {
    ...
    const req = makeRequest(
      "/api/catalog/docs?url=" +
        encodeURIComponent("https://github.com/org/repo"),
    );
    await GET(req);
