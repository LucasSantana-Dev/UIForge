...
for pattern in "${SECRET_PATTERNS[@]}"; do
  ...
  if grep -r -i "$pattern" --include="*.yml" --include="*.yaml" --include="*.json" --include="*.js" --include="*.ts" . | grep -v "REPLACE_WITH_" | grep -v "#.*example" | grep -v "//.*example" | grep -v "SECURITY NOTICE" | grep -v "placeholder" | grep -v "example" | grep -v "demo" | grep -v "test" | grep -v "sample" | grep -v "documentation" | grep -v "patterns" | grep -v "validation" | grep -v "detection" | grep -v ".gitleaks.yml" | grep -v "SECURITY.md" | grep -v "README.md" | grep -v "workflow" | grep -v "github-actions"; then
