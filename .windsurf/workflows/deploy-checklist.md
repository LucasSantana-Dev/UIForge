...
5. Docker build succeeds: `docker build -t siza-mcp .`
6. Docker run smoke test: `docker run --rm siza-mcp` (should start without errors)
7. If integrating with mcp-gateway:
   - `docker-compose.yml`: `siza` service on port `8026`
   - `scripts/gateways.txt`: `siza|http://siza:8026/sse|SSE`
