-- GitHub PR Tracking
-- Track PRs created from Siza generations
...
-- Add github_pr_id to generations
ALTER TABLE generations
  ADD COLUMN IF NOT EXISTS github_pr_id UUID REFERENCES github_prs(id) ON DELETE SET NULL;

-- Indexes
CREATE INDEX IF NOT EXISTS idx_github_prs_user_id ON github_prs(user_id);
CREATE INDEX IF NOT EXISTS idx_github_prs_project_id ON github_prs(project_id);
CREATE INDEX IF NOT EXISTS idx_github_prs_generation_id ON github_prs(generation_id);
CREATE INDEX IF NOT EXISTS idx_github_prs_state ON github_prs(state);
CREATE INDEX IF NOT EXISTS idx_github_prs_repo_id ON github_prs(repo_id);
